...
public static class AgentCLI
{
    ...
    private static string PyScript(int port)
    {
        ...
        var lines = new List<string>
        {
            ...
            "        pass",
            "",
            "# 端口：优先环境变量 PIXSHELL_BRIDGE_PORT/PIXSHELL_PORT；否则读桥写的 agent_port 文件；",
            "# 都没有才回落主端口 47866。绝不硬编码 8xxx。",
            "PORT = os.environ.get('PIXSHELL_BRIDGE_PORT', os.environ.get('PIXSHELL_PORT', ''))",
            "if not PORT:",
            "    try:",
            "        with open(os.path.join(os.environ.get('APPDATA', ''), 'PixShell', 'agent_port'), 'r', encoding='utf-8') as f:",
            "            PORT = f.read().strip()",
            "    except Exception:",
            "        pass",
            ...
            "                    'serverInfo': {'name': 'pixshell', 'version': '0.1.9'}})",
