...
public static class AgentCLI
{
    ...
    public static void Install(int port)
    {
        try
        {
            ...
            WriteIfChanged(PyPath, PyScript());
    ...
    // =====================================================================
    private static string PyScript()
    {
        ...
        var lines = new List<string>
        {
            ...
            "        subprocess.Popen([APP_EXE], close_fds=True, creationflags=DETACHED)",
            "        return True",
            ...
            "        log('HTTP %s %s -> %s' % (e.code, path, body))",
            "        return {'error': 'HTTP %s: %s' % (e.code, body)}",
            "    except Exception as e:",
            ...
            "                log('重试 HTTP %s %s -> %s' % (e2.code, path, b2))",
            "                return {'error': 'HTTP %s: %s' % (e2.code, b2)}",
            "            except Exception as e2:",
            "                log('重试后仍失败 %s: %s' % (path, e2))",
            "                return {'error': '调用 PixShell 桥失败: %s（已自动拉起 App，仍失败）' % e2}",
            "        log('调用桥异常 %s: %s' % (path, e))",
            "        return {'error': '调用 PixShell 桥失败: %s（已尝试自动拉起 App）' % e}",
            "",
            "def pick(d, *keys):",
            ...
            "    if name == 'type_text':",
            "        call('shell', {'session': s, 'text': a.get('text', '') + '\\n'})",
            ...
            "    elif c == 'type':",
            "        call('shell', {'session': s, 'text': ' '.join(rest) + '\\n'})",
            "        import time; time.sleep(0.6)",
            ...
            "    cli(sys.argv[1:]); raise SystemExit",
            "",
            ...
            "                    'serverInfo': {'name': 'pixshell', 'version': '0.1.5'}})",
            "    elif method == 'tools/list':",
            "        reply(rid, {'tools': TOOLS})",
            "    elif method == 'tools/call':",
            "        p = msg.get('params') or {}",
            "        reply(rid, {'content': [{'type': 'text', 'text': call_tool(p.get('name', ''), p.get('arguments') or {})}]})",
            "    elif method == 'ping':",
            "        reply(rid, {})",
