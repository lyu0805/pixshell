...
## 目标
在 `C:\PixShell-win` 建一个**能编译能运行**的 WPF 纵向切片，作为 Windows 原生 SSH 客户端的地基。这一步**不接 SSH**，只证明：WPF 窗口 + WebView2 承载 xterm.js 终端 + 渲染彩色 banner。对标 mac 侧已跑通的同款切片。
...
## 要求
1. 项目：`C:\PixShell-win\PixShell.csproj`，`net9.0-windows`，`<UseWPF>true</UseWPF>`，`<OutputType>WinExe</OutputType>`。
2. NuGet 依赖：
...
## 验收（必须自测）
- 在 `C:\PixShell-win` 运行 `dotnet build`，**必须绿**，把所有错误修掉。
- 把 `dotnet build` 的最终输出写到 `C:\PixShell-win\_build_result.txt`。
