...
enum AiSshBridge {
    ...
    private static func wrapperScript() -> String {
        ...
        let lines: [String] = [
            ...
            "ARGS=( \"$@\" )",
            "HOST=\"\"; USER=\"\"; PORT=\"\"; REMOTE_CMD=(); SAW_DOUBLE=0; WANT_TTY=0",
            "i=0",
            "while [ $i -lt ${#ARGS[@]} ]; do",
            "  a=\"${ARGS[$i]}\"",
            "  case \"$a\" in",
            "    --) SAW_DOUBLE=1; i=$((i+1)); while [ $i -lt ${#ARGS[@]} ]; do REMOTE_CMD+=(\"${ARGS[$i]}\"); i=$((i+1)); done; break ;;",
            "    -p) i=$((i+1)); PORT=\"${ARGS[$i]:-}\" ;;",
            "    -p*) PORT=\"${a#-p}\" ;;",
            "    -t|-tt) WANT_TTY=1 ;;",
            "    -*) ;;",
            ...
            "",
            "fallback() {",
            "  log \"fallback real ssh host=${HOST:-?} args=$*\"",
            "  exec \"$REAL_SSH\" \"$@\"",
            "}",
            "",
            "# 无 host / 显式要走系统：直接回落",
            ...
            "",
            "# ---- 走 PixShell 交互桥 ----",
            "log \"bridge route user=${USER:-} host=$HOST port=${PORT:-} remote_cmd=${#REMOTE_CMD[@]}\"",
            "",
            "# 有远端命令：在当前已连接会话上 exec（不新建系统 ssh）",
            "if [ ${#REMOTE_CMD[@]} -gt 0 ]; then",
            "  CMD=\"${REMOTE_CMD[*]}\"",
            "  # 尝试让会话对准目标主机（best-effort；失败仍 exec 当前会话）",
            "  \"$PIXSHELL\" exec \"$CMD\"",
            "  exit $?",
            "fi",
            "",
