...
enum AgentCLI {
    ...
    private static func script(port: Int) -> String {
        ...
        let lines = [
            ...
            "# 拉起 App：bundle 路径（生成时注入）→ 按名字 → 常见安装位置，逐个试到 open 成功。",
            "launch_app() {",
            "  log \"桥未就绪，自动拉起 PixShell\"",
            "  if [ -n \"$APP_PATH\" ] && [ -d \"$APP_PATH\" ]; then open \"$APP_PATH\" >/dev/null 2>&1 && return 0; fi",
            "  open -a PixShell >/dev/null 2>&1 && return 0",
            "  for p in \"$HOME/Applications/PixShell.app\" /Applications/PixShell.app; do",
            "    [ -d \"$p\" ] && open \"$p\" >/dev/null 2>&1 && return 0",
            "  done",
            "  return 1",
            "}",
