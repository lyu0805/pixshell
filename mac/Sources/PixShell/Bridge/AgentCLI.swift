...
enum AgentCLI {
    ...
    private static func script(port: Int) -> String {
        ...
        let lines = [
            ...
            "set -euo pipefail",
            "umask 077",
            "# 端口：优先环境变量 PIXSHELL_BRIDGE_PORT；否则读桥写的 agent_port 文件（池避让后的实际端口）；",
            "# 都没有才回落主端口 47866。绝不硬编码 8xxx。",
            "PORT=\"${PIXSHELL_BRIDGE_PORT:-}\"",
            "if [ -z \"$PORT\" ] && [ -r \"$HOME/Library/Application Support/PixShell/agent_port\" ]; then",
            "  PORT=\"$(cat \"$HOME/Library/Application Support/PixShell/agent_port\" 2>/dev/null || true)\"",
            "fi",
            "case \"$PORT\" in ''|*[!0-9]*) PORT='47866' ;; esac",
            ...
            "  if [ -n \"$APP_PATH\" ] && [ -d \"$APP_PATH\" ]; then open -n \"$APP_PATH\" --args --headless >/dev/null 2>&1 && return 0; fi",
            "  open -n -a PixShell --args --headless >/dev/null 2>&1 && return 0",
            "  for p in \"$HOME/Applications/PixShell.app\" /Applications/PixShell.app; do",
            "    [ -d \"$p\" ] && open -n \"$p\" --args --headless >/dev/null 2>&1 && return 0",
            "  done",
            "  return 1",
            "}",
            ...
            "do_exec() {",
            "  local r=$(post exec \"$(mkjson \"$SESSION\" cmd \"$*\")\")",
            "  local out=$(echo \"$r\" | field output text stdout 2>/dev/null || true)",
            "  local to=$(echo \"$r\" | python3 -c 'import json,sys; d=json.load(sys.stdin); print(d.get(\"timedOut\",\"\"))' 2>/dev/null || true)",
            "  if [ \"$to\" = \"True\" ]; then",
            "    echo \"[PixShell 超时: 命令 $* 超过 30s 未返回，可能被远端网络/防火墙拦截或 ControlPath 太长]\"",
            "  fi",
            "  [ -n \"$out\" ] && printf '%s\n' \"$out\"",
            "}",
            ...
            "req=urllib.request.Request('http://127.0.0.1:'+os.environ.get('PIXSHELL_BRIDGE_PORT', '47866')+'/v1/app/hosts',headers={'Authorization':'Bearer '+tok})",
            ...
            "  sftp-upload)",
            "    [ $# -ge 2 ] || { echo '用法: pixshell sftp-upload <本地路径> <远端路径>' >&2; exit 2; }",
            "    post sftp/upload \"$(python3 -c 'import json,sys; print(json.dumps({\"session\":int(sys.argv[1]),\"local\":sys.argv[2],\"remote\":sys.argv[3]}))' \"$SESSION\" \"$1\" \"$2\")\" | field remotePath path ;;",
            "  sftp-download)",
            "    [ $# -ge 1 ] || { echo '用法: pixshell sftp-download <远端路径> [本地路径]' >&2; exit 2; }",
            "    post sftp/download \"$(python3 -c 'import json,sys; d={\"session\":int(sys.argv[1]),\"remote\":sys.argv[2]}; d[\"local\"]=sys.argv[3] if len(sys.argv)>3 else \"\"; print(json.dumps(d))' \"$SESSION\" \"$1\" \"${2:-}\")\" | field localPath path ;;",
            "  pwd) echo \"$(cwd_cur)\" ;;",
            "  cd)",
            ...
            "    echo '  sftp-upload <本地路径> <远端路径>  上传文件'",
            "    echo '  sftp-download <远端路径> [本地路径]  下载文件'",
            "    echo '  pwd                显示当前远端目录'",
            "    echo '  cd <目录>          切换当前远端目录'",
