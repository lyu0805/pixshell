...
extension AppDelegate {
    ...
    @objc func checkUpdate() {
        setStatus("检查更新…")
        AppUpdate.check(current: "0.2.0") { [weak self] result in
            guard let self = self else { return }
            ...
            case .updateAvailable(let v):
                ...
                var info = "当前 0.2.0，来源 GitHub Releases（lyu0805/pixshell）。"
            ...
            case .latest:
                self.alert("已是最新", "当前 0.2.0 已是最新版本（GitHub Releases）")
            case .unknown:
                let a = NSAlert.pix()
                a.messageText = "检查更新"
