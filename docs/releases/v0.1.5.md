...
- **rust-analyzer 集成**：诊断（错误/警告实时红线）、悬停提示、补全、定义跳转，双端一致
...
- **监控侧栏细化**：进程 TOP / 磁盘行背景铺满（不再裁切）、表头列宽加大、系统信息按钮居中、Sparkline 柱状图坐标修复（从底部升起）+ FinalShell 风格密集柱
...
- **rust-analyzer integration**: diagnostics (live error/warning underlines), hover, completion, go-to-definition — consistent across platforms
...
- **Monitor sidebar refinement**: process TOP / disk row backgrounds now stretch full-width (no clipping), wider table headers, centered System Info button, Sparkline bar-mode coordinate fix (bars rise from the bottom) with FinalShell-style dense bars
